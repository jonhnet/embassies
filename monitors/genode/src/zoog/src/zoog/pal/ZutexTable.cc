...
ZutexTable::ZutexTable(MallocFactory *mf, SyncFactory *sf)
{
	this->mf = mf;
...
void ZutexTable::wait(ZutexWaitSpec *specs, uint32_t count, ZutexWaiter *waiter)
{
	uint32_t i;
	bool all_match = true;
	...
	for (i=0; i<count; i++)
	{
		...
		uint32_t *host_zutex = (uint32_t*) guest_zutex;
...
//	waiter->reset_event();
	// TODO we declared that as an auto-reset event, so resetting explicitly
	// shouldn't be needed.
	_unlock();
